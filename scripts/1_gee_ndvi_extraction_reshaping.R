...
##' produce the graph of max of mean ndvi by year fct resolution ---------------
## extract mean ndvi
dat_ <- sites.ndvi %>% select(site, year, source, mean_buff_000000m, mean_buff_000500m, mean_buff_008000m) %>%
  gather(mean.ndvi.buff, mean.ndvi.val, mean_buff_000000m, mean_buff_000500m, mean_buff_008000m) %>%
  mutate(buff.size = sub("^mean_buff_[0]{1,5}", "", mean.ndvi.buff)) %>% select(-mean.ndvi.buff)
...
## keep only the max of mean ndvi over buffer by year, site and source
dat_ <- dat_ %>% group_by(site, year, source, buff.size) %>% 
  filter(mean.ndvi.val == max(mean.ndvi.val, na.rm = TRUE)) %>%
  distinct
...
gg <- ggplot(data = dat__, aes(x = year, y = mean.ndvi.val, ymin = mean.ndvi.val - stdDev.ndvi.val,
...
output.dir.bis <- file.path(output.dir, "trends_in_mean_ndvi_site_by_site")
